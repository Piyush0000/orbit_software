      ...
      <section>
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            <div className="bg-rose-500/10 p-2.5 rounded-xl shadow-sm border border-rose-500/20">
              <AlertTriangle className="size-5 text-rose-600" />
            </div>
            <div>
              <h2 className="text-xl font-black tracking-tight">Critical Focus</h2>
              <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Requires Immediate Action</p>
            </div>
          </div>
          <Badge variant="outline" className="bg-rose-50 dark:bg-rose-950/30 text-rose-600 border-rose-200 animate-pulse">
            High Priority
          </Badge>
        </div>
      ...
      <section className="bg-card rounded-xl border shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-center">
            <thead>
              <tr className="bg-muted/30 text-muted-foreground uppercase tracking-widest text-[10px] font-black">
                <th className="text-left py-5 px-6 min-w-[220px]">Performance Metric</th>
                <th className="py-5 px-6">Current Week</th>
                <th className="py-5 px-6 opacity-70">1W Ago</th>
                <th className="py-4 px-6 opacity-50">2W Ago</th>
                <th className="py-4 px-6 opacity-30">3W Ago</th>
              </tr>
            </thead>
                ...
                <tr key={i} className="group hover:bg-muted/10 transition-colors">
                  <td className="text-left py-4 px-6">
                    <div className="flex items-center gap-3">
                       ...
                       <span className="font-bold text-foreground/80 group-hover:text-foreground transition-colors">{row.metric}</span>
                    </div>
                  </td>
                  <td className="py-4 px-6">
                    <div className="flex flex-col items-center">
                      <div className={`font-black text-base ${row.positive ? 'text-emerald-600 dark:text-emerald-400' : ''} ${row.negative ? 'text-rose-600 dark:text-rose-400' : ''}`}>{row.w1}</div>
                      <div className="text-[10px] text-emerald-500 font-bold flex items-center gap-0.5">
                         <TrendingUp className="size-2.5" /> +2.4%
                      </div>
                    </div>
                  </td>
                  <td className="py-4 px-6 opacity-80">
                    <div className={`font-bold ${row.positive ? 'text-emerald-600 dark:text-emerald-400' : ''} ${row.negative ? 'text-rose-600 dark:text-rose-400' : ''}`}>{row.w2}</div>
                  </td>
                  <td className="py-4 px-6 opacity-60 font-medium">
                    {row.w3}
                  </td>
                  <td className="py-4 px-6 opacity-40 font-medium">
                    {row.w4}
                  </td>
                </tr>
              ))}
