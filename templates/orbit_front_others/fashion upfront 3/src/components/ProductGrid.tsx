...
export default function ProductGrid() {
  ...
  const brands = contextBrands.length > 0 ? contextBrands : ['Upfront', 'Essentials'];
  ...
  const displayedProducts = useMemo(() => {
    ...
    if (enabledFilterSet.has('price') && activeFilters.price.length > 0) {
      filtered = filtered.filter(p => {
        const priceInINR = parseINRToNumber(p.price); 
    ...
    filtered.sort((a, b) => {
      const priceA = parseINRToNumber(a.price);
      const priceB = parseINRToNumber(b.price);
      
      switch (sortBy) {
        case 'price_low': return priceA - priceB;
        case 'price_high': return priceB - priceA;
        case 'newest': return (new Date(b.createdAt || 0).getTime()) - (new Date(a.createdAt || 0).getTime());
        // Handling undefined popularity by defaulting to 0
  ...
  if (loading) {
     return <div className="py-20 text-center bg-[var(--page-bg)]">Loading products...</div>;
  }
