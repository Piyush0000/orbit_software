...
export default function Header() {
  ...
  const displayCategories = categories.length > 0 ? categories.slice(0, 5) : ['Men', 'Women', 'Kids'];
  ...
  return (
    ...
    >
              ...
              <div className="flex flex-col gap-6">
                ...
                {displayCategories.map((category) => (
                    <Link 
                      key={category} 
                      href={`/products?category=${category}`}
                      className="text-3xl font-heading font-bold uppercase text-[var(--text-primary)] hover:text-[var(--highlight)]"
      ...
      {/* Mega Menu Overlay - kept as is but mapped to dynamic categories if needed, for now just hiding if no activeCategory */}
      {/* Note: MegaMenu component likely expects specific props, leaving it as is for now but it might use hardcoded data inside */}
      {activeCategory && (
        <MegaMenu
