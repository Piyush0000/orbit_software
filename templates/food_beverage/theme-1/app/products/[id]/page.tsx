...
export default function ProductDetailPage() {
    ...
    // Handle both string and number IDs (API uses strings usually)
    const id = params.id as string

    // Loose comparison for string/number id match
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24">
                    ...
                    <div className="space-y-6">
                        <div className="aspect-square relative rounded-none overflow-hidden bg-zinc-50 border border-zinc-100 shadow-[20px_20px_0px_0px_rgba(244,244,245,1)]">
                            ...
                            <img
                                src={product.images && product.images.length > activeImage ? product.images[activeImage] : (product.image || '/placeholder.png')}
                                alt={product.name}
                    ...
                    <div className="flex flex-col gap-8">
                        <div className="space-y-4">
                            <div className="flex items-center gap-4">
                                {product.veg !== undefined && <VegNonVegBadge veg={product.veg} />}
                                <div className="h-4 w-px bg-zinc-200" />
                                <SpiceLevelTag level={(product.spiceLevel as "Mild" | "Medium" | "Spicy" | "None") || "None"} />
                            </div>
                    ...
                    <Tabs defaultValue="description" className="w-full">
                        ...
                        <TabsContent value="ingredients" className="mt-0 outline-none">
                            <div className="max-w-2xl bg-zinc-50 p-10 border border-zinc-100">
                                ...
                                <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 list-none">
                                    {product.ingredients ? formatIngredients(product.ingredients) : <li className="text-zinc-600">Ingredients not listed.</li>}
                                </ul>
                        ...
                        <TabsContent value="nutrition" className="mt-0 outline-none">
                            <div className="max-w-md">
                                {product.nutrition && <NutritionTable nutrition={product.nutrition} />}
                            </div>
                        </TabsContent>
                        ...
                        <TabsContent value="storage" className="mt-0 outline-none">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-4 bg-zinc-900 text-white p-10">
                                    ...
                                    <p className="text-zinc-300 leading-relaxed font-medium">{product.storage?.instructions || "Keep in a cool, dry place."}</p>
                                </div>
                                <div className="space-y-4 border border-zinc-200 p-10">
                                    ...
                                    <p className="text-zinc-900 leading-relaxed font-black text-4xl tracking-tighter uppercase">{product.storage?.bestBefore || "Best before 6 months"}</p>
                                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{product.storage?.disclaimer || "Consumed best when fresh."}</p>
                                </div>
                            </div>
                        </TabsContent>
                    </Tabs>
