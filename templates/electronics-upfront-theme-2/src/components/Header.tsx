...
export default function Header() {
  ...
  return (
    <div className="sticky top-0 z-[100] flex flex-col w-full transition-all duration-300">
      {/* Top Banner */}
      <div
        className="text-xs sm:text-sm font-bold py-2.5 text-center px-4 tracking-wide relative z-[101]"
        style={{
          background: announcement.backgroundColor || 'linear-gradient(to right, var(--accent-blue), var(--accent-purple))',
            ...
            <nav className="hidden md:flex items-center space-x-8">
              ...
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="font-medium transition-colors hover:text-[var(--accent-blue)]"
                  style={{ color: 'var(--header-text-muted)' }}
                >
                  {link.label}
                </Link>
