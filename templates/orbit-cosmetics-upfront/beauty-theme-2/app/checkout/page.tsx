    ...
    const handleCheckout = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const payload = {
                ...
                customerEmail: email,
                shippingAddress: { name: `${firstName} ${lastName}`.trim(), phone, address, city, state, zip },
                billingAddress: { name: `${firstName} ${lastName}`.trim(), phone, address, city, state, zip },
                tax: 0,
                shipping: 0,
                items: cartItems.map((item) => ({
                    ...
                    variantInfo: item.variantName ? { name: item.variantName } : undefined
                }))
            };
    ...
    if (orderPlaced) {
        return (
            <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 text-center animate-in fade-in zoom-in duration-500">
                ...
                {orderNumber && (
                    <p className="text-sm text-muted-foreground mb-6">Order Number: {orderNumber}</p>
                )}
                <Link href="/">
        ...
        <div className="min-h-screen bg-background pt-20 pb-12 px-4 md:px-8">
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="space-y-8">
                    ...
                    {submitError && (
                        <div className="rounded-md border px-4 py-3 text-sm text-red-600">{submitError}</div>
                    )}
