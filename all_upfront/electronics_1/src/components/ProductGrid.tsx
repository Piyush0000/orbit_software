...
export default function ProductGrid() {
  ...
  const displayTitle = customization?.productSections?.title || "Our Products";
  const displaySubtitle = customization?.productSections?.subtitle || "Discover our curated collection of premium products";
  ...
  const displayedProducts = useMemo(() => {
    ...
    // Remote filter by category from customization
    if (customization?.productSections?.categoryFilter) {
        const filter = customization.productSections.categoryFilter.toLowerCase();
        filtered = filtered.filter(p => p.category.toLowerCase().includes(filter));
    }
    ...
    return filtered;
  }, [products, loading, activeFilters, searchQuery, sortBy]);
        ...
        <div className="w-full">
          {loading ? (
            <div className="text-center py-20">
              ...
              <p className="mt-4 text-lg" style={{ color: 'var(--text)' }}>Loading products...</p>
            </div>
