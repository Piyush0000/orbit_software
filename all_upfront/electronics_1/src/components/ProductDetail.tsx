...
export default function ProductDetail({ productId }: { productId: number }) {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        setLoading(true);
        // Convert productId to string if needed
        const productData = await StorefrontAPI.getProduct(productId.toString());
  ...
  const priceNum = product && product.price ? parseInt(product.price.replace(/[₹,\s]/g, ''), 10) || 0 : 0;
  ...
  // For related products, we'll fetch similar products from the API
  // Since we can't access the full products list anymore, we'll need to fetch related products separately
  // For now, we'll skip this functionality until we enhance the API to support it
    ...
    return (
      <div className="py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="mt-4 text-lg" style={{ color: 'var(--text)' }}>Loading product...</p>
        </div>
      </div>
    );
  }
