    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-4 py-16">
        <div className="max-w-md w-full text-center">
          ...
          {orderId && (
            <p className="text-sm mb-6 font-mono bg-muted rounded px-3 py-2 inline-block" style={{ color: 'var(--text-muted)' }}>
              Order ID: {orderId}
            </p>
          )}
          <p className="text-sm mb-8" style={{ color: 'var(--text-muted)' }}>
            A confirmation will be sent to <strong>{form.email}</strong>. We'll notify you when your order is shipped.
          </p>
          <Link
            href="/"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-lg font-semibold text-lg transition-opacity hover:opacity-90"
  ...
  return (
    <div className="py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {error && (
          <div className="mb-6 rounded-lg border border-red-400 bg-red-50 text-red-700 px-4 py-3 text-sm">
            {error}
          </div>
        )}
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="rounded-lg border p-6" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* First Name */}
                  <div>
                    <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text)' }}>First Name *</label>
                    <input
                  ...
                  {/* Last Name */}
                  <div>
                    <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text)' }}>Last Name *</label>
                    <input
                  ...
                  {/* Phone */}
                  <div className="sm:col-span-2">
                    <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text)' }}>Phone Number *</label>
                    <input
                  ...
                  {/* Address */}
                  <div className="sm:col-span-2">
                    <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text)' }}>Street Address *</label>
                    <textarea
                  ...
                  {/* City */}
                  <div>
                    <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text)' }}>City *</label>
                    <input
                  ...
                  {/* State */}
                  <div>
                    <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text)' }}>State *</label>
                    <input
                  ...
                  {/* PIN Code */}
                  <div>
                    <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text)' }}>PIN Code *</label>
                    <input
              ...
              <div className="rounded-lg border p-6" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    { value: 'upi', label: 'UPI', desc: 'Pay via GPay, PhonePe, Paytm, etc.' },
                    { value: 'card', label: 'Credit / Debit Card', desc: 'Visa, Mastercard, RuPay' },
                  ].map((opt) => (
                    <label
                      key={opt.value}
                      className="flex items-center p-4 rounded-lg border cursor-pointer transition-colors"
                      style={{ borderColor: form.paymentMethod === opt.value ? 'var(--header-text)' : 'var(--card-border)' }}
                    >
                      <input
                        type="radio" name="paymentMethod" value={opt.value}
                        checked={form.paymentMethod === opt.value}
                        onChange={handleChange} className="mr-3 accent-current"
                      />
                      <div>
                        <span className="font-semibold" style={{ color: 'var(--text)' }}>{opt.label}</span>
              ...
              <div className="rounded-lg border p-6 sticky top-24" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
                ...
                <div className="space-y-3 mb-6 pb-6 border-b" style={{ borderColor: 'var(--card-border)' }}>
                  {cartItems.map((item) => (
                    <div key={item.id} className="flex items-start gap-3">
                      <div className="w-14 h-14 rounded-lg overflow-hidden border bg-gray-100 flex-shrink-0">
                        {item.image ? (
                ...
                <div className="space-y-2 mb-6 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span style={{ color: 'var(--text)' }}>{formatINR(subtotal)}</span>
                  </div>
                  {discountAmt > 0 && (
                    <div className="flex justify-between">
                      <span style={{ color: 'var(--text-muted)' }}>Discount</span>
                      <span className="text-green-600">-{formatINR(discountAmt)}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span style={{ color: 'var(--text)' }}>{formatINR(tax)}</span>
                  </div>
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full px-6 py-3 rounded-lg font-semibold text-lg transition-opacity hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  style={{ backgroundColor: 'var(--header-text)', color: 'var(--header-bg)' }}
                >
                  {submitting ? (
                    <>
                ...
                <Link
                  href="/cart"
                  className="flex items-center justify-center gap-1 mt-4 text-sm hover:underline"
