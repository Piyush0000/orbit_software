...
export default function Hero() {
    ...
    const { storeInfo, customization, loading } = useStoreContext()

    // Default slides if no customization data
    const defaultSlides = [
        {
            src: "/hero.png",
    ...
    const heroTitle = customization?.heroSection?.title || "Fresh Food & Drinks";
    const heroSubtitle = customization?.heroSection?.subtitle || "Delivered.";
    const heroDescription = customization?.heroSection?.description || 
        "Premium snacks, refreshing beverages & ready-to-eat meals. Sourced fresh, packed with care, delivered fast to your doorstep.";
                ...
                <div className="grid gap-6 lg:grid-cols-2 lg:gap-12 items-center">
                    <div className="flex flex-col justify-center space-y-6 text-center lg:text-left">
                        <div className="space-y-4">
                            <h1 className="text-4xl font-black tracking-tighter sm:text-5xl md:text-6xl xl:text-8xl/none text-zinc-900 uppercase leading-[0.9]">
                                {heroTitle} <br />
                                <span className="text-primary italic">{heroSubtitle}</span>
                            </h1>
                    ...
                    <div className="relative mx-auto w-full max-w-[600px] lg:max-w-none group">
                        ...
                        <div className="absolute -bottom-6 -left-6 bg-white p-6 shadow-2xl z-30 hidden md:block border border-zinc-100">
                            ...
                            <p className="text-xl font-black uppercase tracking-tighter text-primary">{weeklySpecial}</p>
                            <p className="text-xs font-bold text-zinc-500 mt-1">Starting at {specialPrice}</p>
                        </div>
                    </div>
                </div>
            </div>
