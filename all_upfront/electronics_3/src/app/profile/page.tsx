...
function ProfileContent() {
            ...
            case 'overview':
                return (
                    <div className="space-y-6 animate-fadeIn">
                        <h2 className="text-xl font-bold mb-4 pb-2" style={{ color: 'var(--text-highlight)', borderBottom: '1px solid var(--header-border)' }}>Overview</h2>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="glass-panel p-6 rounded-lg text-center cursor-pointer hover:bg-white/10 transition-colors">
                                <h3 className="text-2xl font-bold" style={{ color: 'var(--accent-cyan)' }}>0</h3>
                                <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Orders</p>
                            </div>
                            <div className="glass-panel p-6 rounded-lg text-center cursor-pointer hover:bg-white/10 transition-colors">
                                <h3 className="text-2xl font-bold" style={{ color: 'var(--accent-cyan)' }}>{wishlist.length}</h3>
                                <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Wishlist</p>
                            </div>
                            <div className="glass-panel p-6 rounded-lg text-center cursor-pointer hover:bg-white/10 transition-colors">
                                <h3 className="text-2xl font-bold" style={{ color: 'var(--accent-cyan)' }}>0</h3>
                                <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Coupons</p>
                            </div>
                        </div>

                        <div className="rounded-lg p-6 mt-8 flex justify-between items-center tech-border" style={{ backgroundColor: 'var(--card-bg)' }}>
                            <div>
                                <h3 className="font-bold text-lg" style={{ color: 'var(--text-highlight)' }}>Detailed Profile</h3>
                                <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Complete your profile to get personalized recommendations.</p>
                            </div>
                            <button onClick={() => setActiveTab('profile')} className="px-4 py-2 rounded-md font-bold text-sm" style={{ background: 'var(--btn-primary-bg)', color: 'var(--btn-primary-text)' }}>Edit Profile</button>
                        </div>
                    </div>
                );
            case 'profile':
            ...
            case 'orders':
                return (
                    <div className="animate-fadeIn">
                        ...
                        <div className="rounded-lg p-8 text-center tech-border" style={{ backgroundColor: 'var(--card-bg)' }}>
                            <div className="w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: 'rgba(102, 252, 241, 0.1)' }}>
                                <svg className="w-10 h-10" style={{ color: 'var(--accent-cyan)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
                            </div>
                            <h3 className="text-lg font-bold mb-2" style={{ color: 'var(--text-highlight)' }}>No active orders properly</h3>
                            <p className="mb-6 max-w-md mx-auto" style={{ color: 'var(--text-muted)' }}>There are no recent orders to show. Why not explore our new collection?</p>
                            <Link href="/#products" className="px-8 py-3 rounded text-sm font-bold uppercase tracking-wider transition-colors inline-block" style={{ background: 'var(--btn-primary-bg)', color: 'var(--btn-primary-text)' }}>Start Shopping</Link>
                        </div>
    ...
    return (
        <div className="min-h-screen flex flex-col transition-colors duration-300" style={{ backgroundColor: 'var(--page-bg)' }}>
            <Header />
            <main className="flex-grow pt-10 pb-20 px-4 sm:px-6 lg:px-8" style={{ backgroundColor: 'var(--page-bg)' }}>
                <div className="max-w-6xl mx-auto">

                    {/* Profile Header Block */}
                    <div className="mb-8 pb-8" style={{ borderBottom: '1px solid var(--header-border)' }}>
                        <div className="flex justify-between items-end">
                            <div>
                                <h1 className="text-2xl font-bold mb-1" style={{ color: 'var(--text-highlight)' }}>Account</h1>
                                <p className="text-sm" style={{ color: 'var(--text-muted)' }}>John Doe</p>
                            </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-8">
                        {/* Sidebar */}
                        <div className="w-full md:w-64 flex-shrink-0 pb-8 md:pb-0 md:pr-8" style={{ borderBottom: '1px solid var(--header-border)', borderRight: '1px solid var(--header-border)' }}>
                            <nav className="space-y-1">
                                ...
                                <div className="my-4" style={{ borderTop: '1px solid var(--header-border)' }}></div>
                                <button
                                    onClick={handleLogout}
                                    className="w-full text-left px-4 py-3 text-sm font-medium border-l-4 border-transparent hover:text-red-500"
...
export default function ProfilePage() {
    return (
        <Suspense fallback={<div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: 'var(--page-bg)' }}><div style={{ color: 'var(--text)' }}>Loading...</div></div>}>
            <ProfileContent />
        </Suspense>
    );
}
