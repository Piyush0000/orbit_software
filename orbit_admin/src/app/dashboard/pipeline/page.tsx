...
import { fetchLeads, updateLeadStatus, deleteLead } from "@/services/leadService";
...
function LeadDetail({
    ...
    const statusOptions: { value: LeadStatus; label: string }[] = [
        { value: "leads" as LeadStatus, label: "Leads" },
        { value: "contacted" as LeadStatus, label: "Contacted" },
        { value: "won" as LeadStatus, label: "Won" },
        { value: "lost" as LeadStatus, label: "Lost" },
    ];
                                    ...
                                    {statusOptions.map((option) => (
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                (lead.status?.toLowerCase() === option.value.toLowerCase())
...
export default function PipelinePage() {
    ...
    const filteredLeads = leads.filter((lead) => {
        ...
        if (dateRange.start && dateRange.end) {
            ...
            const leadDate = toDate(lead.createdAt || lead.timestamp);
            const time = leadDate.getTime();
    ...
    const getPipelineColumns = (): PipelineColumn[] => {
        const columns: PipelineColumn[] = [
            { id: "leads" as LeadStatus, title: "Leads", leads: [] },
            { id: "contacted" as LeadStatus, title: "Contacted", leads: [] },
            { id: "won" as LeadStatus, title: "Won", leads: [] },
            { id: "lost" as LeadStatus, title: "Lost", leads: [] },
        ];

        filteredLeads.forEach((lead) => {
            const status = (lead.status || "leads").toLowerCase() as LeadStatus;
            const column = columns.find((col) => col.id.toLowerCase() === status);
            if (column) {
                column.leads.push(lead);
            } else {
                // Fallback for case sensitivity or exact matches
                const exactColumn = columns.find((col) => col.id === status.toUpperCase());
                if (exactColumn) {
                    exactColumn.leads.push(lead);
                } else {
